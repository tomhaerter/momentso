...
type DateTimeLike = string | Date | Temporal.Instant

function toInstant(dt: DateTimeLike): Temporal.Instant {
...
export function useTimeEntryFormatters() {
  function formatEntryTime(entry: { startTime?: DateTimeLike | null; endTime?: DateTimeLike | null }): string {
  ...
  function formatEntryDuration(entry: { startTime?: DateTimeLike | null; endTime?: DateTimeLike | null }): string {
    if (!entry.startTime || !entry.endTime) return ""
  ...
  function getEntryDurationSeconds(entry: { startTime?: DateTimeLike | null; endTime?: DateTimeLike | null }): number {
    if (!entry.startTime || !entry.endTime) return 0
    return Math.floor(toInstant(entry.endTime).since(toInstant(entry.startTime)).total("seconds"))
  }

  function getEntryDate(entry: { startTime?: DateTimeLike | null }): string {
  ...
  function formatDateInputValue(dt: DateTimeLike | null | undefined): string {
    if (!dt) return ""
    const timeZone = Temporal.Now.timeZoneId()
  ...
  function toTimeInputValue(dt: DateTimeLike | null | undefined): string {
    if (!dt) return ""
    const timeZone = Temporal.Now.timeZoneId()
    const zdt = toInstant(dt).toZonedDateTimeISO(timeZone)
  ...
  function compareDateTimes(a: DateTimeLike, b: DateTimeLike): number {
